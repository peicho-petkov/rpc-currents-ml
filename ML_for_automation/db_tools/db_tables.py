...
class TrainingDataTable(dbTable):
    def __init__(self,tablename='CMS_RPC_PVSS_TEST.MLTRAININGDATA'):
        ...
        self.set_uxc_rh()
        self.set_uxc_dp()
        self.set_acc_int_charge_coll()
        self.set_acc_int_charge_nocoll()
        #self.set_integrated_lumi()
        #self.set_hours_without_lumi()
        
    def get_insert_data_query(self,chdate,imon,vmon,chid,flag,inst_lumi,p,t,rh,acc_int_charge_coll,acc_int_charge_nocoll):
        query="INSERT INTO {table} ({chid_col}, {ichange_col}, {imon_col}, {vmon_col}, {flag_col}, {p_col}, {t_col}, {rh_col}, {inst_col}, {acc_int_charge_coll_col}, {acc_int_charge_nocoll_col}) VALUES ('{chid}','{ichange}','{imon}','{vmon}','{flag}','{p}','{t}','{rh}','{instl}','{acc_ich_c}','{acc_ich_nc}')".format(table=self.tablename,
            chid_col=self.chid, ichange_col=self.change_date, imon_col=self.imon, vmon_col=self.vmon,flag_col=self.flag, p_col=self.uxcP, t_col=self.uxcT, rh_col=self.uxcRH, inst_col=self.instant_lumi, acc_int_charge_coll_col=self.acc_int_charge_coll, acc_int_charge_nocoll=self.acc_int_charge_nocoll,
            chid=chid, ichange=chdate, imon=imon, vmon=vmon, flag=flag, p=p, t=t, rh=rh, instl=inst_lumi, acc_ich_c=acc_int_charge_coll, acc_ich_nc=acc_int_charge_nocoll)
        return query
    ...
    def set_dpid_col(self,name='DPID',type="mediumint not null"):
        self.dpid=name
        self.add_coll(name,type)

    def set_chid_col(self,name='CHAMBER_ID',type="mediumint not null"):
        self.chid=name
        self.add_coll(name,type)
    ...
    def	set_acc_int_charge_coll(self, name="ACC_INT_CHARGE_COLL", type="float default null"):
        self.acc_int_charge_coll=name
        self.add_coll(name,type)
    
    def	set_acc_int_charge_nocoll(self, name="ACC_INT_CHARGE_NOCOLL", type="float default null"):
        self.acc_int_charge_nocoll=name
        self.add_coll(name,type)
    ...
    def get_get_all_chids_query(self):
        query = f"select distinct {self.chid} from {self.tablename}"
        return query
...
class MLModels(dbTable):
    ...
    def set_chid(self,name="CHAMBER_ID",type="int not null"):
        self.chid=name
        self.add_coll(name,type)
    ...
    def get_insert_query(self,modelconf_id,chid,r2,mse,model_path,mojo_path, active_val=0):
        query = f"INSERT INTO {self.tablename} ({self.modelconf_id}, {self.chid}, {self.r2}, {self.mse}, {self.model_path}, {self.mojo_path}, {self.active}) VALUES ('{modelconf_id}','{chid}','{r2}','{mse}','{model_path}','{mojo_path}','{active_val}')"
        return query
    
    def get_update_model_query(self, model_id, modelconf_id, chid, r2, mse, model_path, mojo_path):
        query = f"UPDATE {self.tablename} SET {self.modelconf_id} = '{modelconf_id}', {self.chid} = '{chid}', {self.r2} = '{r2}', {self.mse} = '{mse}', {self.model_path} = '{model_path}', {self.mojo_path} = '{mojo_path}' WHERE {self.model_id} = '{model_id}'"
        return query
    
    def get_model_query(self,modelconf_id,chid):
        query = f"select * from {self.tablename} where {self.modelconf_id} = '{modelconf_id}' and {self.chid} = '{chid}'"
        return query
    ...
    def get_set_active_query(self,active_val, modelconf_id,chid):
        query = f"UPDATE {self.tablename} SET {self.active} = '{active_val}' WHERE {self.modelconf_id} = '{modelconf_id}' and {self.chid} = '{chid}'"
        return query
    ...
    def get_get_chid_by_model_id_query(self, model_id):
        query = f"select {self.chid} from {self.tablename} where {self.model_id} = '{model_id}'"
        return query 
    ...
    def get_get_active_model_ids_and_chids_query(self):
        query = f"select {self.model_id}, {self.chid} from {self.tablename} where {self.active}=1"
        return query 

    def get_get_confid_chid_for_mid_query(self, model_id):
        query = f"select {self.modelconf_id}, {self.chid} from {self.tablename} where {self.model_id} = '{model_id}'"
        return query
    ...
    def get_count_for_conf_id_chid_query(self, modelconf_id, chid):
        query = f"select count(*) from {self.tablename} where {self.modelconf_id} = '{modelconf_id}' and {self.chid} = '{chid}'"
        return query 

    def get_get_chids_by_modelconf_id_query(self, modelconf_id):
        query = f"select {self.chid} from {self.tablename} where {self.modelconf_id} = '{modelconf_id}'"
        return query
    
    def get_get_model_id_for_confname_and_chid_query(self, confid, chid):
        query = f"select {self.model_id} from {self.tablename} where {self.modelconf_id} = '{confid}' and {self.chid} = '{chid}'"
        return query

class MLModelsConf(dbTable):
    #def __init__(self, tablename='MLModelsConf'):
    def __init__(self, tablename='CMS_RPC_PVSS_TEST.MLMODELSCONF'):
        super().__init__(tablename)
        self.add_coll("modelconf_id","bigint auto_increment primary key")
        ...
        self.add_coll(self.last_update, "timestamp not null")
    ...
    def set_flags(self,name="FLAGS",type="VARCHAR(4096) not null"):
        self.flags=name
        self.add_coll(name,type)
    ...
    def get_insert_query(self, name, mlclass, flags, input_cols, output_cols, train_from, train_to, test_from, test_to):
	    query = f''' INSERT INTO {self.tablename} ({self.name}, {self.mlclass}, {self.flags}, {self.input_cols}, {self.output_cols}, {self.train_from}, {self.train_to}, {self.test_from}, {self.test_to}) VALUES ('{name}', '{mlclass}', '{flags}', '{input_cols}', '{output_cols}', TO_TIMESTAMP('{train_from}', 'YYYY-MM-DD'), TO_TIMESTAMP('{train_to}', 'YYYY-MM-DD'), TO_TIMESTAMP('{test_from}', 'YYYY-MM-DD'), TO_TIMESTAMP('{test_to}', 'YYYY-MM-DD')) '''
	    return query
...
class ConfigurationTable(dbTable):
    def __init__(self, tablename='MLCONFIGURATION'):
        super().__init__(tablename)
        self.add_coll("rec_id","bigint auto_increment primary key")
        self.add_coll("LAST_UPDATE","timestamp not null")
