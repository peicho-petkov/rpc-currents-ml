...
class MLModelsConfManager:
    ...
    def RegisterMLModelConf(self, ml_model_descr):
        ...
        return res_dict[self._mlmodelsconftab.rec_id]
    
    def UpdateRegistedMLModelConf(self, ml_model_descr):
        query = self._mlmodelsconftab.get_select_query_by_model_name(ml_model_descr.name)
        res = self._connector.fetchall_for_query_self(query)
        if len(res) != 1:
            return -2
        
        query = self._mlmodelsconftab.update_by_name_query(ml_model_descr.name, ml_model_descr.mlclass,ml_model_descr.input_cols,ml_model_descr.output_cols,ml_model_descr.train_from,ml_model_descr.train_to,ml_model_descr.test_from,ml_model_descr.test_to)
        self._connector.execute_commit_query_self(query)
        return 0
...
class MLModelsManager:
    ...
    def UpdateRegistedMLModelConf(self, ml_model):
        ...
        query = self._mlmodelstab.update_model_query(ml_model.model_id, ml_model.modelconf_id, ml_model.dpid, ml_model.r2, ml_model.mse, ml_model.model_path, ml_model.mojo_path)
        self._connector.execute_commit_query_self(query)
        return 0
